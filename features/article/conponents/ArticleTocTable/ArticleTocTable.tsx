  ...
  return (
    toc.length > 0 ? (
      ...
      <div className="p-9 border-solid border-2 border-[#CED1D9] rounded-lg">
        ...
        <ul>
          {toc.map((data) => (
            <li key={data.id} className={data.name === "h2" ? "font-bold mb-1.5 bg-anker-icon bg-[2px] bg-no-repeat pl-8 last:mb-0" : "font-medium mb-2.5 bg-anker-icon bg-13 bg-[33px] bg-no-repeat pl-14 last:mb-0"}>
              <Link href={`#${data.id}`} className="hover:underline">{data.text}</Link>
            </li>
          ))}
        </ul>
      </div>
